...
def write_spice_contents_and_circuit_json_responses_to_csv():
    models = ["claude-sonnet-4.6", "gemini-3-flash-preview", "gpt-5.4-nano", "llama-3.3-70b-instruct"]
    ...
    for model in models: 
        ...
        for project_key in project_files_dict:
            # print (f"\n--- For project: {project_key} ---\n")
            with open(f'{project_files_dict[project_key]["parent_directory"]}/results/spice_contents_and_circuit_json/{model}_{project_key}.csv', 'w', newline='') as file:
                fields = ['Model', 'Question', 'Category', 'Actual_Response', 'Predicted_Response']
                writer = csv.DictWriter(file, fieldnames=fields)
                writer.writeheader()  # Writes the header row
            ...
            for idx in range(0, 60): 
                ...
                try:
                    # Load the results JSON file
                    results_file = f"{project["parent_directory"]}/results/spice_contents_and_circuit_json/{model}/{category}/{idx+1}.json"
                ...
                with open(f'{project_files_dict[project_key]["parent_directory"]}/results/spice_contents_and_circuit_json/{model}_{project_key}.csv', 'a', newline='') as file:
                    writer = csv.writer(file)
                    # row = {'Model': f'{model}', 'Question': f'{idx+1}', 'Actual_Response': f'{questions[idx]["answer"]}', 'Predicted_Response': f'{result["response"]["answer"]}'}
                    row = [f'{model}', f'Q{idx+1}', f'{category}', f'{questions[idx]["answer"]}', f'{predicted_responses[idx]}']
                    writer.writerow(row)
            ...
            with open(f'{project["parent_directory"]}/results/spice_contents_and_circuit_json/{model}_{project_key}.csv', 'a', newline='\n') as file:
                writer = csv.writer(file)
                writer.writerow(row)
                row = [f'{accuracy}', f'{precision}', f'{recall}', f'{f1}']
                writer.writerow(row)
...
def write_spice_json_and_circuit_contents_responses_to_csv():
    models = ["claude-sonnet-4.6", "gemini-3-flash-preview", "gpt-5.4-nano", "llama-3.3-70b-instruct"]

    # project_files = project_files.meshinger_project({})
    ...
    for model in models: 
        ...
        for project_key in project_files:
            # print (f"\n--- For project: {project_key} ---\n")
            with open(f'{project_files[project_key]["parent_directory"]}/results/circuit_contents_and_spice_json/{model}_{project_key}.csv', 'w', newline='') as file:
                fields = ['Model', 'Question', 'Category', 'Actual_Response', 'Predicted_Response']
                writer = csv.DictWriter(file, fieldnames=fields)
                writer.writeheader()  # Writes the header row
            ...
            for idx in range(0, 60): 
                ...
                try:
                    # Load the results JSON file
                    results_file = f"{project["parent_directory"]}/results/circuit_contents_and_spice_json/{model}/{category}/{idx+1}.json"
                    ...
                    print(e)
                    predicted_responses.append("N/A")
                with open(f'{project_files[project_key]["parent_directory"]}/results/circuit_contents_and_spice_json/{model}_{project_key}.csv', 'a', newline='') as file:
                    writer = csv.writer(file)
                    # row = {'Model': f'{model}', 'Question': f'{idx+1}', 'Actual_Response': f'{questions[idx]["answer"]}', 'Predicted_Response': f'{result["response"]["answer"]}'}
                    row = [f'{model}', f'Q{idx+1}', f'{category}', f'{questions[idx]["answer"]}', f'{result["response"]["answer"]}']
                    writer.writerow(row)
            ...
            with open(f'{project["parent_directory"]}/results/circuit_contents_and_spice_json/{model}_{project_key}.csv', 'a', newline='\n') as file:
                writer = csv.writer(file)
                writer.writerow(row)
                row = [f'{accuracy}', f'{precision}', f'{recall}', f'{f1}']
                writer.writerow(row)
...
if __name__ == "__main__":
    ...
    write_dry_run_responses_to_csv()